...
# Analysis of the Logarithm of the Target Variable
...
# In some variables, missing values indicate that the house does not have that particular feature.

no_cols = ["Alley","BsmtQual","BsmtCond","BsmtExposure","BsmtFinType1","BsmtFinType2","FireplaceQu",
           "GarageType","GarageFinish","GarageQual","GarageCond","PoolQC","Fence","MiscFeature"]

# Filling the empty spaces in the columns with the expression 'No'
for col in no_cols:
...
# Filling empty spaces with median or mean
def quick_missing_imp(data, num_method="median", cat_length=20, target="SalePrice"):
    ...
    # If the variable's object and the number of classes are equal to or less than cat_lengthe, fill the missing values with the mode.
    data = data.apply(lambda x: x.fillna(x.mode()[0]) if (x.dtype == "O" and len(x.unique()) <= cat_length) else x, axis=0)

    # If the num_method is mean, the missing values of non-object variables are filled with the mean.
    if num_method == "mean":
        data = data.apply(lambda x: x.fillna(x.mean()) if x.dtype != "O" else x, axis=0)
    # If the num_method is median, the missing values of non-object variables are filled with the median.
...
# Inclusion of classes into other classes according to their proportions
# ExterCond : 5
...
# cv time with y: 16.2s
# cv time with scaled y: 13.8s
